...
class LockIconWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.is_locked:
            ...
            painter.setBrush(self.theme_colors["icon"])
        painter.setPen(pen)
...
class EyeCareTimerWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        if self.config.get("remember_position") and self.config.get("window_x") is not None and self.config.get("window_y") is not None:
    ...
    def save_config(self):
        if self.config_file_path:
            try:
                # 只保存 DEFAULT_CONFIG 中定义的键，过滤掉 QColor 等运行时对象
                config_to_save = {k: self.config.get(k) for k in DEFAULT_CONFIG.keys()}
    ...
    def mouseReleaseEvent(self, event):
        ...
        if not self.is_locked:
            ...
            if self.config.get("remember_position"):
                self.save_config()
        ...
        try:
            ...
            if command == "update_config":
                if isinstance(value, dict):
                    ...
                    if "theme" in value and value["theme"] != old_theme:
                        self.set_theme(value["theme"])
                    if "font_family" in value:
                        self.set_font(value["font_family"])
                    if "rest_image_opacity" in value:
                        self.rest_widget.animation.setEndValue(value["rest_image_opacity"])
                    if "window_width" in value or "window_height" in value:
                        self.base_config["window_width"] = value.get("window_width", self.base_config["window_width"])
                        self.base_config["window_height"] = value.get("window_height", self.base_config["window_height"])
                        self.apply_scale()
                    if "fade_duration" in value:
                        self.rest_widget.animation.setDuration(value["fade_duration"])
                    self.work_seconds = self.config["work_minutes"] * 60
                    self.rest_seconds = self.config["rest_minutes"] * 60
                    self.log_message("Configuration updated via IPC.")
            elif command == "set_theme" and value in THEMES: self.set_theme(value)
...
def run_app(config=None, config_file_path=None):
    ...
    main_widget = EyeCareTimerWidget(config, config_file_path)
    main_widget.show()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Eye Care Helper")
    ...
    if config_path and os.path.exists(config_path):
        try:
            ...
            print(f"[EyeCareApp]: Loaded config from {config_path}")
        except Exception as e:
            print(f"[EyeCareApp]: Failed to load config file: {e}")
    run_app(loaded_config, config_path)
